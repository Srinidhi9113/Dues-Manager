...
export default function Modal({handleExit,txnError,enterTransaction,handleSubmit,enterAmt}) {
    return (
      <div className='absolute top-10 left-1/2 transform -translate-x-1/2 w-1/2 bg-white border-2 border-gray-200 z-50 rounded-xl p-5'>
            ...
            <label className='text-lg'>Transaction ID:{txnError!=="" && <span className='text-red-600 text-xs'>{txnError}</span>}</label>
            <input className='p-1 rounded-lg w-full border-2 border-gray-200' onChange={enterTransaction}/>
            <label className='text-lg'>Amount: </label>
            <input className='p-1 rounded-lg w-full border-2 border-gray-200' onChange={enterAmt}/>
