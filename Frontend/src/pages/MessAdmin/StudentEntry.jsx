...
export default function StudentsEntry() {
    ...
    const handleRoomChange = (e)=>{
        // console.log(e.target.value)
        const tempstudent = student
        tempstudent["roomID"] = e.target.value
        setStudent(tempstudent)
        // console.log(FilterObjects({id,name,author,pub}))
        const tempList = FilterObjects(tempstudent)
        setShow(tempList)
    ...
    const handleEntry = ()=>{
        if(!idEntry || !nameEntry || !roomEntry){
            setError("Enter all details")
        }
        else{
        const tempList = students
        tempList.push({studentID:idEntry,studentName:nameEntry,roomId:roomEntry})
    ...
    const handleImpose = ()=>{
        if(!room || !amount || !date) setError("Enter all details")
        else{
            setImpose(-1)
        }
    }
    ...
    <div className='w-full p-2 relative'>
        {
            ...
            <div className='bg-white rounded-md border-2 border-gray-400 p-3'>
                ...
                <input className='w-full h-8 border-2 border-gray-300 px-2 rounded-md' onChange={(e)=>{setRoom(e.target.value);setError("")}}></input>
      ...
      <div className='grid grid-rows-1 grid-cols-3 p-2 border-2 border-gray-300 rounded-md gap-2'>
        ...
        <div className=''>
            <input className='w-full h-10 rounded-md px-2 py-1' placeholder='Room Number' onChange={handleRoomChange}/>
        </div>
      </div>
      {showStudents && showStudents.map((student,index)=>{
        return <div className='grid grid-cols-3 bg-white my-2 gap-2 rounded-md hover:cursor-pointer' onClick={()=>setImpose(index)} key={index}>
            ...
            <div className='px-2 py-1 rounded-md my-1 text-lg'>{student.roomID}</div>
        </div>
      })}
...
<div className='absolute bottom-5 right-5'>
        ...
        <div className='bg-white rounded-md p-3 w-72 absolute bottom-16 right-5'>
            ...
            <label>Room Number:</label>
            <input className='w-full h-8 rounded-md border-2 border-gray-400 px-2' onChange={(e)=>{setRoomEntry(e.target.value);setError("")}}/>
